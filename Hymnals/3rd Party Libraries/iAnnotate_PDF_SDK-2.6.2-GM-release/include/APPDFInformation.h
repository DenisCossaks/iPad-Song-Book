...
/**
 * Initializes the PDF Information using a provided sqlite3
 * handle. The handle *must* remain valid for the entire lifetime of
 * this object; note that the object may live after you've released a
 * reference to it. Do not use the handle after passing it to this
 * initializer; this handle must only be used by the APPDFInformation
 * object once it is passed. If you need simulatenous access to the
 * database, create a new handle for other uses.
 *
 * This is not recommended unless you require it to work with a custom
 * sqlite3 implementation. The full sqlite3 API must be properly
 * supported by this custom implementation.
 */
-(id)initWithRawDatabaseHandle:(sqlite3 *)handle;
 ...
 *
 * This method will return YES whenever -allText will return a 
 * non-empty array.  This means that this method will return YES if *any*
 * characters are found in the document, even if they may not be visible in 
 * all contexts . Some PDFs include "hidden" text in various ways and for 
 * various reasons (for example accessibility).  For this reason, this 
 * method may or may not be appropriate for deciding whether markup
 * annotation tools (such as highlighter) and text selection
 * functionality should be enabled.  If a more restrictive set of criteria
 * is needed, see the -hasCharactersNotInSet:inPageRange: method.
 *
 * Furthermore, this <i>alone</i> should <b>not</b> be used to enable 
 * search functionality; that requires that the PDF is processed, so 
 ...
 * \sa APPDFInformation::hasCharactersNotInSet:inPageRange:
 * \sa APPDFInformation::isProcessed
 * \sa APPDFProcessor
 */
-(BOOL)hasText;
...
/**
 * Used to determine if there is any text embedded withinin a single page
 ...
 * APPDFViewOptions::liveProcessingDisabled is turned on).
 *
 * This method will return YES whenever -textOnPage: will return a 
 * non-empty array.  This means that this method will return YES if *any*
 * characters are specified for the page, even if they may not be visible in 
 * all contexts . Some PDFs include "hidden" text in various ways and for 
 * various reasons (for example accessibility).  For this reason, this 
 * method may or may not be appropriate for deciding whether markup
 * annotation tools (such as highlighter) and text selection
 ...
 * is needed, see the -hasCharactersNotInSet:inPageRange: method.
 * 
 * Furthermore, this <i>alone</i> should <b>not</b> be used to enable search 
 * functionality for a single page; that requires that the PDF is processed, 
 * so APPDFInformation::isProcessed * must also be YES in this case.
 *
 * \param page The 0-based page index on which to check for vector text.
 *
...
/**
 * Can be used to determine if there is any selectable text on a single page
 * of the PDF based on a caller's specific criteria.  Similar to 
 * APPDFInformation::hasTextOnPage:, but allows the caller to specify exactly 
 * what text should be excluded.  For example, some callers might want to users 
 * the inversion of the alphanumericCharacterSet.  Other users might not want 
 * to include the symbolCharacterSet and punctuationCharacterSet before inverting.
 *
 * This method is can be used for checking if markup annotation tools (such as highlighter) 
 * and text selection functionality should be enabled.
 *
 * \param set  A set of characters that do not qualify as selectable.
 * \param pageRange A range of 0-based page indices on which to check for characters not in set.
 *
 * \return <code>YES</code> if any characters not in set are detected on the specified 
 * page of the PDF document. Note that some PDF files that appear to be textual
 * may not have embedded text; for example, if a scanned document has
 * not been OCR'd.
 *
 * \sa APPDFInformation::hasTextOnPage:
 */
-(BOOL)hasCharactersNotInSet:(NSCharacterSet *)set inPageRange:(NSRange)pageRange;
