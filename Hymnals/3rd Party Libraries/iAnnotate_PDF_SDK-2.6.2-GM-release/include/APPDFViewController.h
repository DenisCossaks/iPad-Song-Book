...
/**
 * Displays a color picker that originates from the specified view, starting with the given color.
 * If allowAlpha is YES the color picker will let the user specify an alpha value.
 * The rect and view parameters work the same as UIActionSheet::showFromRect:inView
 * The delegate method APAnnotatingPDFViewDelegate::pdfController:didPickColor: will be called
 * when the color picker is dismissed.
 *
 * \sa APAnnotatingPDFViewDelegate::pdfController:didPickColor:
 */
-(void)showColorPickerFromRect:(CGRect)rect inView:(UIView *)view animated:(BOOL)animated withColor:(UIColor *)color allowAlpha:(BOOL)allowAlpha;


/**
 * Displays a color picker that originates from the specified BarButtonItem, starting with the given color.
 * If allowAlpha is YES the color picker will let the user specify an alpha value.
 * The delegate method APAnnotatingPDFViewDelegate::pdfController:didPickColor: will be called
 * when the color picker is dismissed.
 *
 * \sa APAnnotatingPDFViewDelegate::pdfController:didPickColor:
 */
-(void)showColorPickerFromBarButtonItem:(UIBarButtonItem *)item animated:(BOOL)animated withColor:(UIColor *)color allowAlpha:(BOOL)allowAlpha;
...
/**
 * Hides the rectangle highlighting a search result.
 *
 * \param animated <code>YES</code> to animate the transition,
 * <code>NO</code> to make the transition immediate.
 */
-(void)hideSearchHighlightAnimated:(BOOL)animated;
...
/**
 * Used to determine the page at the given view location.
 *
 * You can attach gesture recognizers to APPDFViewController::gestureView then
 * use this method to find the page number of any touch events or completed gestures.
 */
-(NSUInteger)pageIndexAtViewLocation:(CGPoint)location;
